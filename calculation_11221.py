...
# =============================
HR_buffer = []
HR_BUFFER_SIZE = 3  # ★ 이동평균 개수 3개로 축소
HR_JUMP_LIMIT = 25  # ★ 허용 변화폭 25 bpm
...
def smooth_hr(raw_hr, prev_hr):
    """
    raw_hr : 센서에서 직접 읽은 BPM
    prev_hr : 이전 측정에서 '확정된' HR 값
    """

    # ----- 1) 첫 회차: 이전값 없음 → 원본 그대로 사용 -----
    if prev_hr is None:
        HR_buffer.clear()
...
def main():
    ...
    while True:
        ...
        sleep_time = predict_sleep_time(drink_log, personal_half_life, R, alpha, beta, 0)
