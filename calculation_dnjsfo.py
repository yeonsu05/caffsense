...
def smooth_hr(raw_hr, prev_raw_hr):
    ...
    if abs(raw_hr - prev_raw_hr) > HR_JUMP_LIMIT:        # 튀는 값 무시 과정
        return prev_raw_hr
...
def plot_final_graph():
    plt.figure(figsize=(12, 7))                         # 그래프 창의 크기 : 가로 12 세로 7
...
def predict_sleep_time(drink_log, HALF_LIFE_HOURS, R_now, alpha, beta, gamma):
    ...
    if not drink_log:                                   # 카페인 섭취 자체가 없다는 뜻
        return "이미 취침 가능"

    total_dose = sum(D for (t, D) in drink_log)        
    k = math.log(2) / (HALF_LIFE_HOURS * 3600)           

    C_sleep_threshold = max(total_dose * 0.35, 10.0)    # 카페인 농도가 35% 이하면 수면가능, 너무 적은 양을 먹었을 경우 최소치 10 적용
    R_sleep_threshold = 3.0                             # 각성도 3.0 이하면 → 졸릴 수 있는 상태.
    ...
    lambda_decay = 0.15                                 # R 자체의 자연 감소율, 카페인 반응 외의 “자연 피로” 감소값

    global S_log, beta_log                              # S_log와 beta_log는 지역변수가 아니라 전역에 있는 리스트를 그대로 사용하겠다

    # ----- α 개인화 -----
    if len(S_log) > 0:                                  # S_log에 값이 1개 이상 있을 때만 개인화 계산
        S_peak = max(abs(s) for s in S_log)             # S_peak : 지금까지의 S값들 중에서 절댓값 기준 최대값
        if S_REF > 0:                                   # S_REF = 기준 S값 (평균적인 민감도)
            k_alpha_raw = S_peak / S_REF
        else:
            k_alpha_raw = 1.0                           # S_REF가 0인 이상한 상황 방지용으로 else에서 1.0
        k_alpha_raw = max(0.5, min(1.5, k_alpha_raw))   # 너무 과한 개인화 막음 -> 최소(0.5), 최대 (1.5)
    else:
        k_alpha_raw = 1.0                               # s_log 가 비어있으면 개인화 없이 k=1.0

    alpha_personal = alpha * k_alpha_raw
    alpha_eff = alpha * (1 - W_ALPHA) + alpha_personal * W_ALPHA   # 개인화가 너무 강하면 노이즈 타니까 기본 모델 + 개인화 모델을 섞어서 안정성 확보
    ...
    for step in range(int((future_hours * 3600) / dt)):
        ...
        C = 0.0
        for (ti, Di) in drink_log:                            # (ti, Di) = i번째 음료의 (마신 시각, 카페인 mg)
            if t_future >= ti:
                C += Di * math.exp(-k * (t_future - ti))     #  마신 직후 양 * 지수 감소(exp(-k * Δt)) 적용

        dt_hr = dt / 3600.0

        R = R * math.exp(-lambda_decay * dt_hr) + alpha_eff * C * dt_hr - beta_eff * dt_hr   # 새로운 각성도 = (기존 각성도 자연 감소) + (카페인에 의한 각성 증가) - (회복에 의한 감소)
        if R < 0:
            R = 0                                                 # “각성도”라는 물리적 의미 때문에 최소값 0으로 고정

        if C <= C_sleep_threshold and R <= R_sleep_threshold:      # “수면 가능 조건”을 체크하는 부분
